...
namespace osuCrypto
{
	...
	static inline Sodium::Monty25519 blockToCurve(Block256 b)
	{
		static_assert(Sodium::Monty25519::size == sizeof(Block256), "");
		return Sodium::Monty25519(b.data());
	}

	void MiniSender_Ris::outputBigPoly_moeller(u64 myInputSize, u64 theirInputSize, u64 psiSecParam, PRNG& prng, span<block> inputs, span<Channel> chls)
	{
		...
		//####################### online #########################
		gTimer.setTimePoint("r online start ");
		chls[0].asyncSend(g_k, 2);//send g^k
		...
		auto computeGlobalHash = [&](u64 t)
		{
			...
			for (u64 i = startIdx; i < endIdx; i += stepSizeMaskSent)
			{
				...
				std::vector<Block256> sendBuff(curStepSize);
				...
				for (u64 idx = 0; idx < curStepSize; idx++)
				{
					...
					for (u64 idxBlk = 0; idxBlk < first2Slices; ++idxBlk) //slicing
					{
						BytesFromZZ(point_ri[idxBlk].data(), rep(zzY[idxBlk][idxItem])
							, std::min(sizeof(block), mPolyBytes - sizeof(block)));
					}

					auto B = blockToCurve(decKey.decBlock(point_ri));
					B *= sk;

					RandomOracle ro(/*1*/ 2 * sizeof(block));
					//ro.Update(point_ri);
					ro.Update(B);
					ro.Update(inputs[idxItem]);
					ro.Final(sendBuff[idx]);
				}

				chls[t].asyncSend(std::move(sendBuff));
			}
		};
